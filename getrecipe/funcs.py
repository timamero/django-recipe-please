...
def get_instructions(soup):
    ...
    def get_li_or_p_elements():
        # The ingredients can be found by searching for key words in the class of div or ul
        container = soup.find(['div','ul', 'ol'], class_=re.compile(r'instruction|directions'))
        if container == None:
            return None
        li_tags = container.select('li')
        ...
        while len(li_tags) == 0 and len(p_tags) == 0:
            container = container.find_next(['div','ul', 'ol'], class_=re.compile(r'instruction'), recursive=False)
            if container == None:
                return None
            li_tags = container.select('li')
            p_tags = container.select('p')
...
def get_servings(soup):
    ...
    if soup == None:
        return 0
    servings = ''
    servings_container = soup.find(['div', 'span'], class_=re.compile(f'servings|yield|yields|serves'))
...
def get_cooktime(soup):
    ...
    if soup == None:
        return ""
    container = soup.find_all(['div', 'span', 'li'], class_=re.compile(r'cook.*time|time.*active'))
    if len(container) == 0:
        return 'Not found'
    text_list = []
    for element in container:
